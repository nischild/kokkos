...
concept is_integral_index_range = requires(const Range& r) {
  r.begin();
  r.end();
  r.size();
...
class OffsetView : public View<DataType, Properties...> {
 ...
 public:
  ...
  // (e.g. Kokkos::Array, std::array) satisfying is_integral_index_range. The
  ...
  template <Impl::is_integral_index_range Begins,
            Impl::is_integral_index_range Ends>
  ...
  template <Impl::is_integral_index_range Ends>
  ...
  template <class... P>
    requires(!Kokkos::Impl::ViewCtorProp<P...>::has_pointer)
  explicit OffsetView(const Kokkos::Impl::ViewCtorProp<P...>& arg_prop,
                      const begins_type& begins_, const begins_type& ends_)
      : base_t(arg_prop, compute_layout_from_begins_ends(begins_, ends_)) {
  ...
  template <typename Label>
    requires(Kokkos::Impl::is_view_label<Label>::value)
  explicit OffsetView(const Label& arg_label, const begins_type& begins_,
  ...
  explicit OffsetView(const Label& arg_label,
                      const std::pair<int64_t, int64_t> range0,
